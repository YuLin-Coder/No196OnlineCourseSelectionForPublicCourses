<template>
	<div class="page_root" id="root_demo">
		...
		<div class="warp" style="background-color: white!important;">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                <!-- 标题栏组件 -->
		                <div class="article_recommend" style="background-color: white!important;">
		                    <div class="title">
		                        <span>新闻资讯 </span>
		                    </div>
		                    <div class="more_box">
		...
		<!-- 推荐课程模块(开始) -->
		<div class="warp" style="background-color: #EAF3EE!important;">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/curriculum/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_curriculum = !show_list_curriculum">
		                        <div class="switch_box">
		                            <span v-if="show_list_curriculum"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--课程推荐列表组件 -->
		                <list_curriculum v-if="show_list_curriculum" :list="list_curriculum" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_curriculum.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_curriculum" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_curriculum" :key="i" @click="to_details('curriculum',o,'curriculum_id')">
		                            <td v-for="(oj,n) in fields_curriculum">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				fields_curriculum: [
					...
					{
						key: "limited_number_of_people",
						label: "限定人数",
						type: "数字"
					},
					{
						key: "number_of_people_selected",
						label: "已选人数",
						type: "数字"
					},
				],
		...
		methods: {
			...
			// 获取课程列表
			get_curriculum() {
				let url = "~/api/curriculum/get_list?";
				url = "~/api/curriculum/get_hits_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_curriculum = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_curriculum() {
		        var list = this.list_curriculum;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_curriculum.map((o) => {
		                // 第二个中括号是对象的属性

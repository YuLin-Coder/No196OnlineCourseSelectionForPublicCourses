<template>
	<div class="diy_details_box diy_div_teaching_task">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "teaching_task_id",
					source_id: this.obj["teaching_task_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/teaching_task/set?teaching_task_id=' + _this.obj["teaching_task_id"], {
								praise_len
							}, (res) => {
								if(res.result){

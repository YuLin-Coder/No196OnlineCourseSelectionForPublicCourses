<template>
	<div class="diy_details_box diy_div_inform_students">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "inform_students_id",
					source_id: this.obj["inform_students_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/inform_students/set?inform_students_id=' + _this.obj["inform_students_id"], {
								praise_len
							}, (res) => {
								if(res.result){

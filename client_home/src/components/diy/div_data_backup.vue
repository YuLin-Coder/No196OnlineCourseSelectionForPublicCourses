<template>
	<div class="diy_details_box diy_div_data_backup">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "data_backup_id",
					source_id: this.obj["data_backup_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/data_backup/set?data_backup_id=' + _this.obj["data_backup_id"], {
								praise_len
							}, (res) => {
								if(res.result){

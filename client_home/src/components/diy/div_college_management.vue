<template>
	<div class="diy_details_box diy_div_college_management">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "college_management_id",
					source_id: this.obj["college_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/college_management/set?college_management_id=' + _this.obj["college_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){

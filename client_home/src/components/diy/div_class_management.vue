<template>
	<div class="diy_details_box diy_div_class_management">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "class_management_id",
					source_id: this.obj["class_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/class_management/set?class_management_id=' + _this.obj["class_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){

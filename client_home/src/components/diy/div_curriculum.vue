<template>
	<div class="diy_details_box diy_div_curriculum">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							<div class="row_1" v-if="imgList.length">
									<div class="diy_img" v-for="(item,index) in imgList" :key="item+index" style="max-height: 450px;">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/course_selection_management/edit','add')" @click="to_form('/course_selection_management/edit')" >选课 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/curriculum/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/curriculum/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->

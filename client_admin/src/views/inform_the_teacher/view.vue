		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','title') || $check_field('add','title') || $check_field('set','title')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="标题" prop="title">
					<el-input id="title" v-model="form['title']" placeholder="请输入标题"
							  v-if="user_group === '管理员' || (form['inform_the_teacher_id'] && $check_field('set','title')) || (!form['inform_the_teacher_id'] && $check_field('add','title'))" :disabled="disabledObj['title_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher') || $check_field('add','teacher') || $check_field('set','teacher')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师" prop="teacher">
						<el-select v-if="user_group === '管理员' || (form['inform_the_teacher_id'] && $check_field('set','teacher')) || (!form['inform_the_teacher_id'] && $check_field('add','teacher'))" id="teacher" v-model="form['teacher']" :disabled="disabledObj['teacher_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','video_file_') || $check_field('add','video_file_') || $check_field('set','video_file_')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="视频文件" prop="video_file_">
					<el-upload v-if="user_group === '管理员' || (form['inform_the_teacher_id'] && $check_field('set','video_file_')) || (!form['inform_the_teacher_id'] && $check_field('add','video_file_'))" class="upload-demo" drag
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','enclosure') || $check_field('add','enclosure') || $check_field('set','enclosure')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="附件" prop="enclosure">
					...
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['inform_the_teacher_id'] && $check_field('set','enclosure')) || (!form['inform_the_teacher_id'] && $check_field('add','enclosure'))" class="upload-demo" drag
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','notice_content') || $check_field('add','notice_content') || $check_field('set','notice_content')" :xs="24" :sm="24" :lg="24" class="el_form_editor_warp">
				<el-form-item label="通知内容" prop="notice_content">
					<quill-editor v-model.number="form['notice_content']"
						v-if="user_group === '管理员' || (form['inform_the_teacher_id'] && $check_field('set','notice_content')) || (!form['inform_the_teacher_id'] && $check_field('add','notice_content')) ">
					</quill-editor>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/inform_the_teacher/add?",
				url_set: "~/api/inform_the_teacher/set?",
				url_get_obj: "~/api/inform_the_teacher/get_obj?",
				url_upload: "~/api/inform_the_teacher/upload?",

				query: {
					"inform_the_teacher_id": 0,
				},
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/inform_the_teacher/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/inform_the_teacher/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/inform_the_teacher/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/inform_the_teacher/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/inform_the_teacher/view','get');
